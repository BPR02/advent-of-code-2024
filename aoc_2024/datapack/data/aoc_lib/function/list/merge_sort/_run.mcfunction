...
# get array length
execute store result score lib.list.len aoc_calc run data get storage aoc:register lib.list.to_sort
# return early if the list is already sorted
execute if score lib.list.len.t aoc_calc matches ..1 run return 1

# split array into list of lists with a single element (reverses it too due to push/pop optimization)
data modify storage aoc:register lib.list.sorting set value []
...
# return list
data modify storage aoc:register lib.list.to_sort set from storage aoc:register lib.list.sorting[-1]
